...
4. 存储层只负责数据读写。
5. 提醒层只负责构造、记录和发送提醒。
6. 策略层只负责输出独立、角色化的策略证据。
7. 策略聚合层只负责按 `strategy_role` 综合多个策略证据。
...
23A 后，策略主链路采用“策略角色协作 / 策略接力”，分为三层：

1. 策略层：输出 `StrategyResult` / `StrategySignal`，提供角色化证据；不生成最终 `advice`，不生成完整 `trade_setup`。
2. 聚合层：按 `strategy_role` 汇总 `common_result`，识别方向证据、关键价位证据、风险证据、过滤结果、背景证据、冲突和缺失。
3. 建议层：基于聚合结果、后续模型分析和硬性风控规则，生成最终 `advice`、`trade_setup`、`wait` 或 `stop_trading`。

明确禁止：

1. 策略层直接生成最终 advice。
2. 聚合层硬依赖某个具体策略的私有字段。
3. 公共 schema 按具体 `strategy_name` 膨胀。
4. 将 `gann_angle`、`fibonacci_618`、`support_detection_method` 等策略私有字段写入 `common_result`。

如果某个具体策略关闭或缺少可选 adapter，聚合层应记录 `evidence_missing` 并降级处理，不得导致主链路失败。
...
1. 定义 `BaseStrategy`。
2. 实现具体策略模块。
3. 为策略配置 `strategy_role`。
4. 输出独立、角色化的策略信号。
5. 保留策略私有 payload，但不把私有字段放进公共结果。
...
策略层输出必须是结构化信号。23A 后，优先使用 `StrategyResult` 三段结构，并兼容历史 `StrategySignal`。
...
7. 关键理由。
8. 风险或缺失提示。
9. 策略私有 payload 摘要或引用。

不同角色记录不同公共字段，不要求每个策略都输出方向、入场、止损、目标或完整交易结构。
...
1. 接收多个策略信号。
2. 按 `strategy_role` 汇总证据。
3. 分析策略一致性。
4. 分析策略分歧。
5. 识别证据缺失。
6. 生成方向、价位、风险、过滤、背景等角色摘要。
7. 判断是否需要降级为 `wait` 或 `stop_trading`。
8. 输出聚合理由和影响聚合的证据来源。
...
1. `directional` 证据是否充分。
2. `support_resistance` 证据是否充分。
3. `risk_control` 是否否决或降级。
4. `filter` 是否通过、拒绝或未知。
5. `context` 是否提供了关键背景。
6. 哪些策略证据缺失，需要记录 `evidence_missing`。
