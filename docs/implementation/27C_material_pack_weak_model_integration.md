...
`weak_model_run_id`、`weak_model_aggregation_id`、`quality_check_id` 仍保留在 material pack
中作为审计追踪字段，但不纳入 material fingerprint / 20 复用判断。这些 ID 是流水追踪
ID，重跑 27A/27B 可能产生新 ID，但不一定代表弱模型结论本身发生变化。

因此弱模型强度、质量状态、配置 hash 或 quality issue 的 error_code/severity 变化时，
20 不会静默复用旧 19 审查；单纯 WMR/WMA/WMQC 流水 ID 变化不会被误判为材料变化。
