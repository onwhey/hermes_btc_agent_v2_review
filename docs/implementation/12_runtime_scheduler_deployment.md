...
其中 `SchedulerExecutionSlotStore` 只是兼容导入名，不再提供旧的 `reserve_execution_slot()` 方法；
旧方法无法表达 running / completed / status 分离状态，调用方必须迁移到
`RedisSchedulerSlotStore.acquire_slot_for_run()`。
...
failed / skipped / blocked status marker 的含义是：该 slot 已经得到一个非 completed 的可追踪终态。
默认 TTL 为 86400 秒，意味着同一个 slot 在 TTL 内不会由 scheduler 自动重复执行。
如需人工重跑，应由用户或运维先确认原因，再删除对应的
`scheduler:status:<job>:<slot>` key，或使用相应 CLI 做人工检查 / 手动回补。
scheduler 不自动修复、不自动回补、不自动修改正式 K线表。

stale / expired status marker 只记录 running lock 异常诊断。runner 在原 running lock value 匹配时会删除旧 running lock，并安全重试一次。
如果竞争删除失败，runner 本轮跳过，不会误删其他进程持有的 running lock。
...
```text
scheduler:job:<job>:<slot>
```

不会被新的 slot state 识别为 running lock、completed marker 或 status marker。

从旧版本热更新到新版本时，建议按以下顺序处理：

```text
1. 先停止 scheduler 常驻进程。
2. 检查 Redis 中是否存在 scheduler:job:* 旧 key。
3. 根据当前 slot 实际情况，由用户或运维人工选择：
   - 等待旧窗口结束；
   - 在确认不影响正在运行任务后删除旧 key；
   - 接受一次非破坏性重复调度。
4. 启动新版本 scheduler。
```

如果接受一次非破坏性重复调度，scheduler 可能再次调用 09 / 11 job。
这不会绕过 09 的 Redis 写入锁、幂等写入、冲突检查、连续性检查和数据质量检查，
也不会让 scheduler 直接写正式 K线表。
但它可能产生额外日志、额外 `collector_event_log` 或一次额外的业务结果记录。

旧配置 `SCHEDULER_JOB_SLOT_TTL_SECONDS` 不再被 scheduler runner 读取。
新版本应改用：
