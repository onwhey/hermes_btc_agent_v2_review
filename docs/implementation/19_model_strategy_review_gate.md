...
- thinking mode 下的 `request_params.reasoning_effort`
- thinking mode 下的 `request_params.extra_body.thinking.type=enabled`
- `ignored_params_in_thinking_mode`

`profile_hash` 覆盖上述配置内容，并参与 `review_version_key`。新增或修改 profile 后，
`review_version_key` 会变化。DeepSeek adapter 只使用 profile 中的 `model_name` 和
`request_params` 构造请求；不在 adapter 中写死业务模型名，也不补业务关键默认参数。
如果关键字段缺失，registry 阶段会返回 blocked，不会默默依赖供应商默认值。
...
raw request artifact、DeepSeek thinking mode profile 字段和 ignored params。默认 pytest
