...
1. `binance_rest`：来自 Binance REST 接口。
2. `manual_repair`：人工修复。
3. `system_repair`：系统修复。

4h 标准 K线来源必须是 `binance_rest`。

禁止使用 WebSocket 作为 4h K线标准来源。

周期信息已经由 `interval` 字段表达，不建议在 `data_source` 中写成 `binance_rest_4h`。
...
```text
kline.close_time <= Binance server time
```
