...
第 16 阶段只消费第 15 阶段已经判定合格的 MarketContextSnapshot，不绕过或重写第 15 阶段门禁。第 15 阶段的每日完整复核是低频整体健康背景：状态必须为 `passed` / `healthy` 且记录不能过旧，但不要求每日复核 `end_open_time_ms` 覆盖当前最新 4h K线。最新 4h K线是否可用于快照，仍由第 15 阶段的 K线数量、理论最新已收盘、未收盘、周期边界和连续性检查决定。
