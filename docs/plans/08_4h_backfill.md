...
10. 发现异常时调用 `app/alerting` 发送 Hermes 固定模板报警。
11. 创建对应测试文件。
12. 创建对应实现说明文件。
...
9. 发生异常时调用 `app/alerting` 发送 Hermes 固定模板报警。
...
4. `status`：`running`、`success`、`failed`、`blocked`、`partial_success`。
...
## 20. collector_event_log Repository 要求
