...
```json
{
  ...
  "activation_condition": "4h 收盘重新站上最近压力位上方，并且成交量不明显萎缩",
  "invalidation_condition": "4h 收盘跌破最近 swing low 下方",
...
```json
{
  ...
  "candidate_scenarios": [
    {
      "scenario_type": "long_candidate / short_candidate / wait_candidate / stop_trading_candidate",
      "activation_condition": "候选成立条件",
      "invalidation_condition": "候选失效条件",
...
```text
invalidation_condition 不是交易止损指令
...
```text
是否触发 activation_condition
是否先触发 invalidation_condition
