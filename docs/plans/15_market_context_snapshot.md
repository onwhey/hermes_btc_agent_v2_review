...
说明：每日完整复核是低频审计，不是每根最新 K线的实时通行证。MarketContextSnapshot 仍然必须检查 K线数量、理论最新已收盘 K线、新鲜度、未收盘、周期边界和连续性；每日复核只作为整体健康背景，要求状态为 `passed` / `healthy` 且记录不能过旧，默认不超过 30 小时，但不要求 `latest_daily_quality_check.end_open_time_ms` 覆盖当前最新 4h K线。
...
4. 不自动修复。
5. 最近每日复核记录缺失、创建时间缺失或超过 30 小时时 blocked。
6. 最近每日复核 `end_open_time_ms` 未覆盖当前最新 4h K线时不单独 blocked；每日复核只作为低频整体健康背景。
