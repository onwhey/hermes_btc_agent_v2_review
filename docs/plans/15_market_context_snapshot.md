...
但第 15 阶段生成快照时，应记录实际使用的 lookback_count、起止时间和 K线引用，保证可追溯。
...
2. `snapshot_payload_json` 保存给后续策略层和模型层使用的事实输入摘要。
3. 不在本表中保存策略结论。
4. 不在本表中保存大模型输出。
5. 不在本表中保存交易建议。
...
### 10.2 market_context_snapshot_kline_ref
...
1. 4h 与 1d 引用都写入此表，通过 `interval_value` 区分。
2. `market_kline_id` 分别指向对应周期正式 K线表中的 id。
3. 该表不复制 OHLCV 字段，只记录引用关系。
4. 正式 K线表禁止人工改数和覆盖写，因此引用关系可以用于复盘追溯。
5. 由于 4h 与 1d 是分表，`market_kline_id` 是多态引用；如实现阶段认为跨表外键复杂，可以先不建数据库外键，但必须在 service 层保证引用正确。
...
UNIQUE(snapshot_id, interval_value, sequence_no)
UNIQUE(snapshot_id, interval_value, open_time_ms)
```
...
是否包含 taker_buy 相关字段，以现有 K线表字段为准。
...
5. 如果不是 dry-run 且 confirm-write 为 true，调用 repository 写入 snapshot 和 kline_ref。
...
2. `--dry-run` 不写 snapshot 表，不写 kline_ref 表。
...
6. 写入 `market_context_snapshot_kline_ref`。
7. payload 中包含 4h 与 1d。
8. 不包含策略字段。
9. 不请求 Binance。
10. 不修改 K线表。
...
13. 本阶段没有实现哪些内容。
14. 如何运行测试。
15. 后续策略层如何基于 snapshot_id 读取事实输入。
