...
本阶段交付增量采集 service 和人工 CLI 调试入口。

本阶段保留两种触发来源语义：

1. `trigger_source = cli`：用户通过 `scripts.collect_4h_klines` 手动触发。
2. `trigger_source = scheduler`：后续 scheduler job 直接调用 app service 时传入。

两种触发方式必须通过 `trigger_source` 明确区分。

09 阶段不实现完整 scheduler runner，也不允许 scheduler 调用 scripts；12 阶段负责补生产运行与调度部署，并新增 scheduler job/runner 直接调用 09 app service。

本阶段负责：

1. 创建 4h K线增量采集 service。
2. 创建 4h K线采集脚本入口。
3. 明确未来 scheduler job 直接调用 app service 的边界。
...
4. 覆盖已有配置、日志、数据库、报警、Binance REST、K线基础模块、质量检查模块、回补模块。
5. 用脚手架工具重置项目目录。
...
本阶段实现人工 CLI 触发，并在 service 层保留 scheduler 触发来源语义，供 12 阶段的 scheduler job/runner 直接调用。

### 8.1 scheduler 触发

09 阶段不实现完整调度运行进程。

调度入口不得走 `scripts.collect_4h_klines`。

后续 scheduler 触发增量采集时，只允许通过 scheduler job 直接 import 并调用 app service：

```
12 阶段 scheduler job/runner
    ↓
app/market_data/collector/kline_4h_collector_service.py::run_incremental_4h_collection(trigger_source="scheduler")
```

调度侧不允许通过 subprocess、runpy、`python -m scripts` 或任何脚本转发方式间接调用采集。

scheduler job 直接调用 service 时必须记录：
...
该脚本是 4h K线增量采集的人工 CLI 调试入口，只允许用户手动执行。
...
4. 非法 `--trigger-source` 必须拒绝执行，包括 `scheduler`。
5. 脚本只负责解析参数、初始化配置和调用 service。
6. 脚本不得直接请求 Binance。
7. 脚本不得直接写数据库。
8. 脚本不得直接发送 Hermes。
9. 脚本不得承载核心业务逻辑。
10. 脚本不得被 scheduler 调用，也不得接受 scheduler 作为 CLI 触发来源。
...
## 10. 未来 Scheduler job 接入要求

09 阶段不交付 scheduler job 或常驻 scheduler runner。

12 阶段补生产运行与调度部署时，如新增 09 scheduler job/runner，必须遵守本节边界。
...
6. 不允许 job 执行交易。
7. 不允许 job 调用 `scripts.collect_4h_klines`。
8. 不允许 job 通过 subprocess、runpy、`python -m scripts` 间接调用采集。
...
必须通过 app/alerting 发送 Hermes 固定模板报警
    ↓
CLI 返回非 0 状态码；未来 scheduler job 直接调用 service 时记录失败
```
...
5. CLI 返回非 0 状态码。
6. 未来 scheduler job 直接调用 service 时记录失败。
...
未来 scheduler job 直接调用 service 触发时：
...
09 阶段不实现 scheduler job 或完整 scheduler runtime。12 阶段接入 scheduler 时必须在对应 plan 和 implementation 写清楚：

1. job/runner 入口在哪。
2. 期望调度频率是什么。
3. scheduler 如何直接调用 app service。
4. 不得通过 scripts、subprocess、runpy 或 `python -m scripts` 间接调用采集。
...
```
未来 scheduler job 直接调用 service，或用户 CLI 触发
    ↓
拉取最近若干根
    ↓
...
2. 09 面向最近增量。
3. 08 不允许 scheduler。
4. 09 service 允许未来 scheduler job 直接调用，但必须显式记录 `trigger_source = scheduler`，且不得通过 scripts 调用。
5. 08 和 09 都不允许人工改数。
...
4. service 直接收到 `trigger_source = scheduler` 时映射为 `data_source = binance_rest_by_scheduler`。
5. CLI 显式传入 `trigger_source = cli`。
6. CLI 传入 `trigger_source = scheduler` 时拒绝执行，不进入采集流程。
...
9. 未来 scheduler job 直接调用 service 遇到失败时，应记录失败状态并抛出或返回明确失败结果。
...
1. 本模块入口。
2. CLI 参数。
3. 未来 scheduler job 直接调用 app service 的边界。
4. 未来 scheduler 调用链。
5. CLI 调用链。
6. Binance REST 请求流程。
7. 重叠拉取逻辑。
...
3. CLI 中 `--trigger-source` 非 `cli` 时拒绝执行，包括 `scheduler`。
4. CLI 触发时记录 `trigger_source = cli`。
5. service 被未来 scheduler job 直接调用并传入 `trigger_source = scheduler` 时，记录 `trigger_source = scheduler`。
6. CLI 触发时记录 `data_source = binance_rest_by_cli`。
7. service 被未来 scheduler job 直接调用并传入 `trigger_source = scheduler` 时，记录 `data_source = binance_rest_by_scheduler`。
8. `pytest` 默认可以运行成功。
9. 默认测试不请求真实 Binance。
10. 默认测试不连接真实 MySQL。
...
29. 文档不再把 `scripts.collect_4h_klines` 表达为调度入口。
...
2. 查看 CLI 是否拒绝 scheduler 作为触发来源。
...
18. 运行测试。
19. 运行 help 命令。
20. 查看调度路径是否误用 `scripts.collect_4h_klines`、subprocess、runpy 或 `python -m scripts`。
