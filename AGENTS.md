...
7. Codex 编写代码时必须按 Python 3.11.x 编写，禁止使用 Python 3.12+ 专属语法。项目必须使用独立虚拟环境，不得复用 Hermes 自身 venv。
...
## 36. 数据库字段长度与大字段人工审核规则

数据库字段设计必须保持简洁、可控、可查询、可审计。不得为了开发方便，将不应长期结构化保存的大段内容直接塞入单个字段。
...
数据库优先保存结构化事实、状态、索引、摘要和关键证据，不应保存无限膨胀的上下文垃圾。
...
8. 大块日志；
9. 可由已有数据重新计算或恢复的内容；
10. 其他可能持续膨胀、难以查询、难以维护的内容。

### 3. 人工审核触发条件

凡是字段内容预计或实际可能满足以下任一条件，必须进入人工审核流程：

1. 超过 32KB；
2. 超过 10,000 个字符；
3. 后续可能持续追加增长；
4. 单字段内容无法明确说明查询价值；
5. 字段只是为了“方便保存完整上下文”而设计；
6. 字段内容可以通过其他表、时间范围、ID、版本号重新恢复；
7. 字段可能被 Codex 或后续自动化流程不断扩大。

未经人工审核确认，不允许新增、扩大或保留此类字段设计。

### 4. 人工审核必须决定处理方式

触发人工审核后，必须明确选择以下处理方式之一：

1. 拆分为结构化字段；
2. 拆分为子表；
3. 只保存摘要；
4. 只保存索引、ID、时间范围、版本号或恢复契约；
5. 截断保存，并记录截断原因；
6. 转为文件或对象存储，并在数据库中只保存引用；
7. 拒绝入库；
8. 明确批准保留大字段，并记录原因、最大长度、清理策略和查询影响。

### 5. 默认判定

任何可能达到几十万字符的字段设计，默认视为数据库设计错误，必须重新审核。

数据库不是文件系统，也不是上下文垃圾桶。除非人工审核明确批准，否则不得通过扩大 Text/JSON 字段来规避结构化建模。

### 6. 对本项目的特别约束

Hermes BTC 项目中，以下内容默认不得直接作为大字段写入数据库：
...
7. 完整模型接力上下文；
8. 完整复盘过程日志；
9. 可以通过 symbol、interval、open_time、snapshot_id、run_id、version 等信息重新恢复的数据。

如确有保存必要，必须先人工审核，并优先采用摘要化、结构化拆表、索引引用或外部存储方案。